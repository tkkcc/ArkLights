...
<!-- - 默认不自动重启acc进程，但这样刷肉鸽每小时内存泄漏44M，一天1G。启用后会定期释放软件内存，但在部分设备上会造成启动失败与闪退。 -->
<!-- - 单号最大剿灭次数默认为1。 -->
...
游戏内异形屏适配设为0，系统关游戏模式，关游戏工具箱，关全局侧边栏，关深色夜间护眼模式，关隐藏刘海，虚拟键要位于右侧，边缘全面屏手势可能影响软件点击。
...
#### 软件设置不能修改/不能输入/卡在输入法选择界面

禁止开启速通输入法。如曾开启过，卸载软件后重装。
...
软件主界面上方为“任务开始前的设置”，中间为“需要执行的任务”，下方为“任务完成后的设置”，底部为“更多功能”。
...
<!-- ::: tip -->
<!-- 长时间挂机时，游戏与软件占用内存会不断上涨(可用top监测到)，因此需要定期重启游戏与软件进程来释放内存。在有root权限的情况下软件才有能力做到这点。 -->
<!-- ::: -->
...
#### 软件关闭状态下自动启动/软件启动后不显示设置界面

未正确关闭软件，或在软件自重启阶段关闭软件。
...
没必要，软件根据实际的 设施数量、设施等级、产物类型、干员技能 来计算，不套用固定组合。
...
不会，一是标签识别非常保守，有问题就保留，二是有官方保险，在未勾选资深标签时招募或刷新都会弹窗，软件遇到会卡住，但理论上不会遇到。
...
任务完成后，如果设了定时，软件会等到下个定时点重启。只是打开软件，但没按`仅定时`或`启动并定时`无效。注意系统时区是不是东八区。
...
<!-- > 先加反馈群，水月头像管理员是机器人。 -->
<!-- > -->
<!-- > 创建群聊将数个机器人拉进群，将群号填到软件主界面上`完成后通知QQ`后。 -->
<!-- > -->

将个人QQ号(需要是机器人好友)或群号(需要是机器人所在群)填到软件高级设置`QQ通知账号`即可。

目前公开机器人由于风控严重已不再提供，请按下面的教程自建。风控主要和好友数群数以及消息内容消息数量相关，少部分人使用不会风控。

也可以使用pushplus推送，支持多种[通知渠道](http://www.pushplus.plus/doc/guide/api.html#%E4%B8%80%E3%80%81%E5%8F%91%E9%80%81%E6%B6%88%E6%81%AF%E6%8E%A5%E5%8F%A3)，但有[额度限制](http://www.pushplus.plus/doc/guide/use.html)。在软件高级设置中填写token即可使用，无需自建服务。自定义webhook编码拼在channel后，即`webhook&webhook=自定义编码`。
...
在一个软件能连接的设备上（模拟器软件能连接宿主机、安卓软件能连接同设备termux、安卓软件能连接有公网IP的服务器），用npm安装[qqimagedeliver](https://github.com/tkkcc/qqimagedeliver)
```sh
...
将`服务器IP地址或域名:服务端口`填到软件高级设置`QQ通知自建服务地址`
```txt
82.156.198.12:49875
```
用自己QQ加该QQ为好友，将自己QQ填到软件高级设置`QQ通知账号`

软件主界面任务全不勾，启动，观察能否收到QQ通知
...
另一种情况是先被抢登回到登录界面，软件抢回（第一次），然后又被抢登或掉线回到登录界面（第二次），依次类推。
...
框架限制。框架为软件提供发短信拨号功能，因此会申请所需权限，即使软件不使用这些功能。不放心可在虚拟机、模拟器、云手机上使用。

#### 为什么申请root权限

有root权限时软件能自动开无障碍、关闭游戏、亮屏熄屏、录入锁屏手势密码、保活。
...
软件通过录屏方式获取屏幕，判断状态，使用无障碍方式进行点击。不碰内存与网络。
...
目前，已有数量相当的使用者将速通投入到了商业使用中。速通并不禁止商业行为，但是对于部分使用者大幅度压低市场价格形成垄断的行为我们表示十分的心寒。此现象尤其体现于集成战略当中，更有甚者在游戏内容尚未更新的情况下就向外许诺能够进行尚未实装的内容托管。这种竭泽而渔的情况是我们所不愿意见到的。因此速通主分支将不再更新水月肉鸽内容，此内容的更新已移动至另一个专有版本。若进行商业大批量使用可以申请专有版本的使用，请发送邮件到`dev@aegirtech.com`，同时在邮件中详细介绍情况和使用场景。
