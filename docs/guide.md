...
```txt
当期委托*5 活动9*99 CA-5 PR-D-2x0 10-17 上一次x0
```
表示5次最新剿灭+99次活动9+1次CA-5+1次10-17。
...
先选+2%制造与+7%贸易干员，防止同类技能干员一起上，剩下按鹰序。
...
将服务器IP与端口按如下格式填到脚本高级设置`QQ通知自建服务地址`
```txt
...
有root权限时脚本能自动开无障碍、关闭游戏、亮屏熄屏、捕捉用户点击位置用于解锁屏幕、进程保活。
