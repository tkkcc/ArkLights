...
function _M.split_message(message_s)
  ...
  if not message.headers and not message.body then message.headers = message_s end
...
function _M.parse_from(from)
  ...
  if not address then _, __, address = string.find(from, "%s*(.+)%s*") end
  name = name or ""
  address = address or ""
...
function _M.parse(mbox_s)
  ...
  for i = 1, #mbox do mbox[i] = _M.parse_message(mbox[i]) end
  return mbox
end
